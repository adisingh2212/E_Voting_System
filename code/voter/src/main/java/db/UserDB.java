...
public class UserDB extends DatabaseManager {
    ...
    public void getNameFromAuthor(int id) throws Exception {
        ...
        String query1 = "SELECT * FROM author WHERE id = 1"; 
    ...
    public void updateAlreadyVoted(AlreadyVoted alreadyVoted) throws Exception {
        connectDatabase();
        String query = "UPDATE already_voted SET " +
                "hasVoted = ?;  ";


        PreparedStatement stmt = getConnection().prepareStatement(query);
        int i = 1;
        stmt.setInt(i++, alreadyVoted.getHasVoted());
